...
내부 라우팅 판단은 실제 호출 player/profile 구성이 바뀔 때만 사용자에게
`선수교체: <profile 또는 lineup> — <짧은 이유>`로 한 번 알립니다. 같은 선수가 계속
진행하거나 follow-up·retry를 수행할 때, 또는 감독이 통합·검증·커밋·푸시를 이어받을
때는 별도 상태 문구를 표시하지 않습니다.
