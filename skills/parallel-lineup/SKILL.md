---
name: parallel-lineup
description: Use when the user explicitly requests parallel agent work, when independent bounded subtasks can run concurrently, or when large disposable research, investigation, audit, or verification context should be isolated from the main conversation. Do not use for tightly coupled work with shared unresolved decisions.
---

# Parallel Lineup

Send multiple players to bat when independence, context isolation, or latency
outweighs coordination and merge risk.

For coding tasks that need implementation ownership, `implementation-at-bat`
...
dispatch an eligible player assigned the review and Coach concurrently.
Omit Coach only when unavailable or when the reviews
